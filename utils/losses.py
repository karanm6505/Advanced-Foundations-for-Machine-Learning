...
class FocalLoss(nn.Module):
    ...
    def forward(self, logit, target):
        return focal_loss(
            F.cross_entropy(
                logit, target, reduction="none", weight=self.weight
            ),
            self.gamma,
        )
...
class LDAMLoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        logit_m = (
            logit - batch_m * self.s
        )  # scale only the margin, as the logit is already scaled.
...
class LADELoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        per_cls_pred_spread = logit.T * (
            target
            == torch.arange(0, self.num_classes).view(-1, 1).type_as(target)
        )  # C x N
        pred_spread = (
            logit
